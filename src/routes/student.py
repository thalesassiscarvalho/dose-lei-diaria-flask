...
from src.models.law import Law, Subject, user_favorite_laws
...
def filter_laws():
    ...
    # A consulta base agora é sobre os TÓPICOS (leis com parent_id), não sobre os diplomas.
    # Isso facilita a filtragem por status e outras propriedades do tópico.
    ...
    # Filtra por concurso, se aplicável
    ...
    if selected_diploma_id:
        ...
        if selected_subject_id:
            topics_query = topics_query.join(Law.subject).filter(Subject.id == selected_subject_id)

    # Filtra por tópico específico (do refinamento)
    ...
    # OTIMIZAÇÃO: A filtragem por status e favoritos agora é feita no banco de dados.
    # Usamos LEFT JOIN para trazer informações de progresso e favoritos.
    # Isso é muito mais rápido do que fazer o loop em Python.
    
    # Junta com a tabela de progresso do usuário
    ...
    if show_favorites:
        topics_query = topics_query.join(user_favorite_laws, and_(
            user_favorite_laws.c.user_id == current_user.id,
            user_favorite_laws.c.law_id == Law.id
        ))

    # Executa a consulta otimizada
    all_filtered_topics = topics_query.all()

    # Agora, em vez de filtrar, apenas agrupamos os resultados já filtrados para a renderização.
    subjects_with_diplomas = {}
    
    # Informações de progresso e favoritos para todos os tópicos do usuário (para cálculo de %)
    ...
    # Agrupando os tópicos por diploma e matéria
    diplomas_map = {}
    for topic in all_filtered_topics:
        ...
        if diploma not in diplomas_map:
            diplomas_map[diploma] = {
                ...
                "children": [],
                # Pega todos os tópicos do diploma (não apenas os filtrados) para calcular o progresso total corretamente
    ...
    for diploma, data in diplomas_map.items():
        ...
        subjects_with_diplomas[subject_name].append({
            ...
            "filtered_children": sorted(data["children"], key=lambda x: x['title']) # Ordena os filhos para exibição
        })
        
    return jsonify(subjects_with_diplomas=subjects_with_diplomas)
# =========================================================================================
# <<< FIM DA ALTERAÇÃO (PASSO 2) >>>
# =========================================================================================
