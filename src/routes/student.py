...
ALLOWED_ATTRIBUTES = {
    '*': ['style', 'class', 'title', 'id', 'data-comment-id'],
    'a': ['href', 'title', 'target'],
    'i': ['class']
}
ALLOWED_STYLES = [
    ...
    'border', 'border-left', 'position', 'top', 'right', 'cursor', 'font-size', 
    'transition', 'opacity', 'transform', 'font-style', 'display'
]
...
def get_user_level_info(points):
    ...
    for i, level in enumerate(LEVELS):
        if points >= level["min_points"]:
            ...
            break
    if not next_level:
        return {"current_level": user_level, "next_level": None, "progress_percent": 100, "points_to_next": 0}
    ...
    return {"current_level": user_level, "next_level": next_level, "progress_percent": progress_percent, "points_to_next": points_for_next_level - points}
...
def get_laws_for_subject(subject_id):
    ...
    laws_query = Law.query.filter(Law.subject_id == subject_id, Law.parent_id.is_(None))
...
def autocomplete_search():
    ...
    if search_type == 'topic' or search_type == 'all':
        topics_query = Law.query.filter(Law.parent_id.isnot(None), or_(Law.title.ilike(search_term), Law.content.ilike(search_term))).options(joinedload(Law.parent))
        if allowed_law_ids is not None:
            topics_query = topics_query.filter(Law.id.in_(allowed_law_ids))
        for topic in topics_query.limit(limit).all():
            parent_title = topic.parent.title if topic.parent else "Tópico"
            results.append({"id": topic.id, "title": f"{parent_title} - {topic.title}", "category": "Tópico de Lei", "url": url_for('student.view_law', law_id=topic.id)})
    if search_type != 'topic':
        if search_type == 'all':
            limit = 5
        laws_query = Law.query.filter(Law.parent_id.is_(None), Law.title.ilike(search_term))
        ...
        for law in laws_query.limit(limit).all():
            results.append({"id": law.id, "title": law.title, "category": "Diploma Legal", "url": url_for('student.dashboard', diploma_id=law.id, subject_id=law.subject_id)})
        for subject in subjects_query.limit(limit).all():
            results.append({"id": subject.id, "title": subject.name, "category": "Matéria", "url": url_for('student.dashboard', subject_id=subject.id)})
    unique_results = []
    seen_ids = set()
    for r in results:
...
def get_concurso_details(concurso_id):
    ...
    return jsonify(success=True, edital_url=concurso.edital_verticalizado_url)
...
def dashboard():
    ...
    recent_progresses_query = UserProgress.query.join(UserProgress.law).filter(UserProgress.user_id == current_user.id, UserProgress.last_accessed_at.isnot(None), Law.parent_id.isnot(None))
    favorite_topics_query = current_user.favorite_laws.options(joinedload(Law.parent).joinedload(Law.subject)).filter(Law.parent_id.isnot(None))
    if allowed_law_ids is not None:
        subjects_query = subjects_query.filter(Subject.id.in_(allowed_subject_ids))
    ...
    recent_progresses = recent_progresses_query.options(joinedload(UserProgress.law).joinedload(Law.parent)).order_by(UserProgress.last_accessed_at.desc()).limit(3).all()
    recent_activities = [{"law": progress.law, "time_ago": _humanize_time_delta(progress.last_accessed_at)} for progress in recent_progresses]
    ...
    non_fixed_announcements = Announcement.query.filter(Announcement.is_active==True, Announcement.is_fixed==False, Announcement.id.notin_(seen_announcement_ids)).order_by(Announcement.created_at.desc()).all()
    ...
    for law, topics in grouped_by_law.items():
        ...
        topic_details_list = [{"id": topic.id, "title": topic.title, "is_completed": topic.id in completed_topic_ids, "is_in_progress": topic.id in in_progress_topic_ids} for topic in sorted(topics, key=lambda t: t.id)]
        favorites_by_subject[subject].append({"parent": law, "topics": topic_details_list, "progress": progress_percentage})
    ...
    total_study_seconds = db.session.query(func.sum(StudySession.duration_seconds)).filter_by(user_id=current_user.id).scalar() or 0
    formatted_total_study = _format_duration(total_study_seconds)
    one_week_ago = datetime.datetime.utcnow() - timedelta(days=7)
    weekly_study_seconds = db.session.query(func.sum(StudySession.duration_seconds)).filter_by(user_id=current_user.id).filter(StudySession.recorded_at >= one_week_ago).scalar() or 0
    ...
    try:
        ...
        logging.warning("Fuso horário 'America/Sao_Paulo' não encontrado. Usando UTC.")
        brazil_tz = pytz.utc
        now_in_brazil = datetime.datetime.utcnow().astimezone(brazil_tz)
    ...
    daily_study_seconds = db.session.query(func.sum(StudySession.duration_seconds)).filter_by(user_id=current_user.id).filter(StudySession.recorded_at >= today_start_utc).scalar() or 0
    ...
    for i in range(7):
        ...
        day_label = "Hoje" if current_day == today_in_brazil_date else days_of_week_br[current_day.weekday()]
        weekly_activity_data.append({"label": day_label, "minutes": duration_minutes})
    study_by_subject_data = db.session.query(Subject.name, func.sum(StudySession.duration_seconds)).join(StudySession, StudySession.subject_id == Subject.id).filter(StudySession.user_id == current_user.id).group_by(Subject.name).order_by(func.sum(StudySession.duration_seconds).desc()).all()
    most_studied_subject = {'name': study_by_subject_data[0][0], 'duration': _format_duration(study_by_subject_data[0][1])} if study_by_subject_data else None
    study_data_for_chart = [{'name': name, 'duration_seconds': duration} for name, duration in study_by_subject_data]
...
def filter_laws():
    ...
    for diploma in all_diplomas:
        ...
        for topic in children_iterator:
            ...
            passes_status = (not selected_status or (selected_status == 'completed' and is_completed) or (selected_status == 'in_progress' and is_in_progress) or (selected_status == 'not_read' and is_not_read))
            ...
            if passes_status and passes_topic and passes_favorite:
                children_to_display.append({"id": topic.id, "title": topic.title, "is_completed": is_completed, "is_in_progress": is_in_progress, "is_favorite": is_favorite})
        if children_to_display:
            ...
            diploma_data = {"title": diploma.title, "progress_percentage": progress_percentage, "subject_name": diploma.subject.name if diploma.subject else "Sem Matéria", "filtered_children": children_to_display}
...
def view_law(law_id):
    ...
    banner_to_show = None
    if law.banner:
        seen_banner_record = UserSeenLawBanner.query.filter_by(user_id=current_user.id, law_id=law_id, seen_at_timestamp=law.banner.last_updated).first()
        if not seen_banner_record:
            banner_to_show = law.banner
    return render_template("student/view_law.html", law=law, is_completed=(progress.status == 'concluido'), last_read_article=progress.last_read_article, current_status=progress.status, is_favorited=is_favorited, display_content=content_to_display, banner_to_show=banner_to_show)
...
def mark_complete(law_id):
    ...
    if not progress or progress.status != 'concluido':
        ...
        if unlocked_achievements_obj:
            ...
            unlocked_achievements = [{"name": ach.name, "description": ach.description, "icon": ach.icon} for ach in unlocked_achievements_obj]
        try:
            db.session.commit()
        except Exception as e:
            db.session.rollback()
    ...
    return jsonify(success=True, unlocked_achievements=unlocked_achievements)
...
def mark_banner_seen(law_id):
    ...
    existing_seen = UserSeenLawBanner.query.filter_by(user_id=current_user.id, law_id=law.id, seen_at_timestamp=banner_timestamp).first()
    if not existing_seen:
        seen_record = UserSeenLawBanner(user_id=current_user.id, law_id=law.id, seen_at_timestamp=banner_timestamp)
...
def handle_comments(law_id):
    ...
    if request.method == "POST":
        ...
        new_comment = UserComment(content=content, anchor_paragraph_id=anchor_id, user_id=current_user.id, law_id=law_id)
...
def _serialize_todo_item(item):
    ...
    return {"id": item.id, "content": item.content, "is_completed": item.is_completed, "category": item.category, "created_at": item.created_at.isoformat() if item.created_at else None, "completed_at": item.completed_at.isoformat() if item.completed_at else None, **law_info}

@student_bp.route("/api/todo_items", methods=["GET"])
@login_required
def get_todo_items():
    todo_items = current_user.todo_items.options(joinedload(TodoItem.law).joinedload(Law.parent)).order_by(TodoItem.is_completed.asc(), TodoItem.created_at.desc()).all()
...
def add_todo_item():
    ...
    new_item = TodoItem(user_id=current_user.id, content=content, category=category, law_id=law_id, is_completed=False)
    try:
        db.session.add(new_item)
        db.session.commit()
        db.session.refresh(new_item)
        return jsonify(success=True, message="Item adicionado!", todo_item=_serialize_todo_item(new_item)), 201
    except Exception as e:
        db.session.rollback()
...
def toggle_todo_item(item_id):
    item = TodoItem.query.options(joinedload(TodoItem.law).joinedload(Law.parent)).filter_by(id=item_id, user_id=current_user.id).first()
    if not item:
    ...
    try:
        ...
        return jsonify(success=True, message=message, todo_item=_serialize_todo_item(item))
    except Exception as e:
        db.session.rollback()
...
def record_study_session():
    ...
    try:
        new_session = StudySession(user_id=current_user.id, law_id=law_id, subject_id=subject_id, duration_seconds=duration_seconds, entry_type=entry_type, start_time=start_time, end_time=end_time, recorded_at=datetime.datetime.utcnow())
...
@login_required
def get_study_stats():
    study_data = db.session.query(Subject.name, func.sum(StudySession.duration_seconds).label('total_duration_seconds')).join(Subject, StudySession.subject_id == Subject.id).filter(StudySession.user_id == current_user.id).group_by(Subject.name).order_by(Subject.name).all()
    stats_by_subject = []
    ...
    for subject_name, duration_seconds in study_data:
        ...
        stats_by_subject.append({'subject_name': subject_name, 'total_seconds': duration_seconds, 'formatted_duration': f"{int(hours)}h {int(minutes)}m {int(seconds)}s"})
    total_hours = total_study_seconds // 3600
    total_minutes = (total_study_seconds % 3600) // 60
    return jsonify(success=True, stats_by_subject=stats_by_subject, total_study_seconds=total_study_seconds, total_formatted_duration=f"{int(total_hours)}h {int(total_minutes)}m")
